...
export default function TeacherEdit() {
    ...
    const fetchTeacher = async () => {
      ...
      try {
        const { data: teacher, error } = await supabase
          .from('teachers')
          .select('*, rfid_cards_teachers(card_number)')
          .eq('id', id)
          .single();
        ...
        if (teacher) {
          setFormData({
            ...
            rfidCardId: teacher.rfid_cards_teachers?.[0]?.card_number || '',
          });
        }
      } catch (error) {
  ...
  if (fetching) {
    return (
      <MainLayout title="Edit Teacher" titleBn="শিক্ষক সম্পাদনা">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground font-bengali">শিক্ষক সম্পাদনা</p>
            </div>
          </div>
        </div>
